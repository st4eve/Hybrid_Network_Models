...
class Calculate_Fisher_Information_Matrix():
    def __init__(
        ...
        Args:
            model_func (Keras Model Initialization Function): Model to be tested
        ...
        self.models = [self.model_func() for _ in range(n_iter)]
        self.test_model = self.model_func()
        self.x_train = np.array(x_train)
        self.test_model(self.x_train[0:1])
        self.n_iter = n_iter
        self.models = [model_func() for _ in range(n_iter)]
        self.inputs = np.random.uniform(-1, 1, size=[n_iter] + list(self.x_train.shape))
    ...
    def process_fishers(self):
        ...
        fishers = np.mean(np.reshape(fishers, [n_iter, num_datapoints, total_params, total_params]), axis=0)
