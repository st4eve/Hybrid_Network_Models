#!/bin/env bash
#SBATCH --array=0-9
#SBATCH --job-name=WINENoisy_Experiment
#SBATCH --output=outputWINENoisy/output%A%a.txt
#SBATCH --error=outputWINENoisy/error%A%a.txt
#SBATCH --mem=16GB
...
noisy=${noisy_values[$(( trial % ${#noisy_values[@]} ))]}
trial=$(( trial / ${#noisy_values[@]} ))
...
python WINE_PWBTest_Experiment.py with activation=${activation} classical_size=${classical_size} cutoff_dimension=${cutoff_dimension} encoding_method=${encoding_method} ff_activation=${ff_activation} max_initial_weight=${max_initial_weight} n_circuits=${n_circuits} n_classes=${n_classes} n_qumodes=${n_qumodes} norm_threshold=${norm_threshold} num_classical=${num_classical} num_layers=${num_layers} regularizer_string=${regularizer_string} seed=${seed} shots=${shots} noisy=${noisy} precision=${precision} max_epoch=${max_epoch} exp_train=${exp_train} 
