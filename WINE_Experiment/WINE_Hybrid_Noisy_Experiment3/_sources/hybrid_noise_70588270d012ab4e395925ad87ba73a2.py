...
EXPERIMENT_NAME = "WINE_Hybrid_Noisy_Experiment3"
...
def confnet_config():
    """Default config"""
    sigma = SIGMAS[-1]  # pylint: disable=W0612
...
def define_and_train(sigma, num_qumodes, network_type):
    ...
    for experiment_num in experiment_numbers:
        ...
        if config["num_qumodes"] == num_qumodes and config["network_type"] == network_type:
            ...
            break
    for i in range(100):
        noise = tf.random.normal(x_val.shape(), stddev=sigma)
        validate_data = [noise + x_val.astype(np.float32),y_val.astype(np.float32)]
        model.load_weights(f"{target_experiment_path}/weights/weight{NUM_EPOCHS-1}.ckpt", by_name=False)
        val_loss, val_acc = model.evaluate(*validate_data, verbose=1)
