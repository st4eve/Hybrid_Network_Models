...
ex.observers.append(FileStorageObserver('Experiment_Data7'))
...
#%% Logs
@ex.capture
def log_performance(_run, logs, epoch, time, model):
    ...
    model.save_weights('Experiment_Data6/%s/weights/weight%d' % (_run._id, epoch))
...
def define_and_train(encoding_strategy, cutoff_dimension, num_layers, num_pre_classical):
    ...
    class LogPerformance(Callback):
        ...
        def on_epoch_end(self, epoch, logs={}):
            ...
            log_performance(logs=logs, epoch=epoch, time=duration, model=self.model)
