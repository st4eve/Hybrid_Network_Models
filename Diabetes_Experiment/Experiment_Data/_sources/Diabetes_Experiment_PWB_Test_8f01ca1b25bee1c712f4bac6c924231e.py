...
import os
import sys
sys.path.append('../Experiment_Data7')
...
def define_and_test(encoding_strategy, cutoff_dimension, num_layers, num_pre_classical):
    ...
    class Net(tf.keras.Model):
        def __init__(self):
            ...
            for i in range(num_pre_classical):
                pre_quantum_network.append(PWBLinearLayer(8,activation="relu",
                                                            constraint=lambda t: tf.clip_by_value(t, -1.0, 1.0)))

            pre_quantum_network.append(layers.PWBLinearLayer(8,
                            constraint=lambda t: tf.clip_by_value(t, -1.0, 1.0)))
            ...
            self.sequential_2 = tf.keras.Sequential([
                self.quantum_layer,
                layers.PWBLinearLayer(4, activation="relu",
                            constraint=lambda t: tf.clip_by_value(t, -1.0, 1.0)),
                layers.PWBLinearLayer(2, activation="sigmoid",
                            constraint=lambda t: tf.clip_by_value(t, -1.0, 1.0))])
    ...
    model.load_weights('../Experiment_Data7/11/weights/weights49')
